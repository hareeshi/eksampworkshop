...
SERVICE_ACCOUNT_NAME=iamproxy-amp
...
#
# Setup the permission policy grants write permissions for all AWS AMP workspaces
#
...
#
# Create a service account and associate it with the IAM role created
#
read -r -d '' SERVICE_ACCOUNT <<EOF
apiVersion: v1
kind: ServiceAccount
metadata:
  name: iamproxy-amp
  ...
  annotations:
    eks.amazonaws.com/role-arn: arn:aws:iam::$AWS_ACCOUNT_ID:role/$SERVICE_ACCOUNT_IAM_ROLE
EOF
